...
public class ScheduleService {
    ...
    public List<Event> generateSchedule(String semesterId, GeneratorConfig config) {
        ...
        // Delete any existing schedule events for this semester before generating new
        // ones
    ...
    private void validateUserCanEditEvent(Event event, User user) {
        ...
        if ("TEACHER".equals(user.getRole())) {
            ...
            boolean isAssigned = mappings.stream().anyMatch(m -> m.getBatchId().equals(event.getBatchId()) &&
    ...
    private void validateAvailability(int week, int day, int period, String roomId, String sectionId,
            String eventIdToIgnore) {
        // Room availability
        List<Event> roomEvents = eventRepo.findByRoomId(roomId);
        boolean roomConflict = roomEvents.stream()
                .anyMatch(e -> e.getWeek() == week && e.getDay() == day && e.getPeriod() == period &&
                        !"CANCELED".equals(e.getStatus()) &&
                        (eventIdToIgnore == null || !e.getId().equals(eventIdToIgnore)));
        if (roomConflict)
            throw new ValidationException("Room is already booked at this time.");

        // Section availability
        List<Event> sectionEvents = eventRepo.findBySectionId(sectionId);
        boolean sectionConflict = sectionEvents.stream()
                .anyMatch(e -> e.getWeek() == week && e.getDay() == day && e.getPeriod() == period &&
                        !"CANCELED".equals(e.getStatus()) &&
                        (eventIdToIgnore == null || !e.getId().equals(eventIdToIgnore)));
        if (sectionConflict)
            throw new ValidationException("Section is already booked at this time.");
    }
    ...
    public Event restoreEvent(String eventId, User user) {
        Event event = eventRepo.findById(eventId).orElseThrow(() -> new ResourceNotFoundException("Event not found"));
        validateUserCanEditEvent(event, user);
        validateAvailability(event.getWeek(), event.getDay(), event.getPeriod(), event.getRoomId(),
                event.getSectionId(), eventId);
    ...
    public Event bookEvent(Event newEvent, User user) {
        ...
        validateAvailability(newEvent.getWeek(), newEvent.getDay(), newEvent.getPeriod(), newEvent.getRoomId(),
                newEvent.getSectionId(), null);
    ...
    public List<EventDto> getAllEvents(String semesterId, String sectionId, String teacherId) {
        ...
        return all.stream()
                .filter(e -> {
                    if (semesterId != null)
                        return semesterId.equals(e.getSemesterId());
                    return semesterRepo.findActive().map(active -> active.getId().equals(e.getSemesterId()))
                            .orElse(false);
                })
    ...
    public EventDto mapToDto(Event current) {
        ...
        if (current.getTeacherId() != null)
            teacherRepo.findById(current.getTeacherId()).ifPresent(t -> eventDto.teacherName = t.getName());
        if (current.getCourseId() != null)
            courseRepo.findById(current.getCourseId()).ifPresent(c -> eventDto.courseName = c.getName());
        if (current.getSectionId() != null)
            batchRepo.findSectionById(current.getSectionId()).ifPresent(s -> eventDto.sectionName = s.getName());
        if (current.getBatchId() != null)
            batchRepo.findById(current.getBatchId()).ifPresent(b -> eventDto.batchName = b.getName());
        ...
        if (teacherId != null) {
            teacherRepo.findById(teacherId).ifPresent(t -> {
                if ("LAB_INSTRUCTOR".equals(t.getType())) {
                    // roomTypePreference = "COMPUTER_LAB"; // Actually the user said "consider the
                    // kind of room the need lab or lecture(based on their teacher type)". Let's
                    // just find rooms that match the type.
                }
            });
        }
        final String teacherType = teacherId != null
                ? teacherRepo.findById(teacherId).map(Teacher::getType).orElse(null)
                : null;
        final String requiredRoomType = "LAB_INSTRUCTOR".equals(teacherType) ? "COMPUTER_LAB" : "LECTURE_ROOM";

        Semester active = semesterId != null
                ? semesterRepo.findById(semesterId).orElse(semesterRepo.findActive().orElse(null))
                : semesterRepo.findActive().orElse(null);
        if (active == null)
        ...
        for (int w = beginWeek; w <= maxWeeks; w++) {
            for (int day = 1; day <= 6; day++) {
                for (int period = 1; period <= 5; period++) {
                    ...
                    boolean personOrSectionBusy = conflicts.stream()
                            .anyMatch(e -> e.getWeek() == currentW && e.getDay() == d && e.getPeriod() == p &&
                    ...
                    if (!personOrSectionBusy) {
                        ...
                        for (Room room : allRooms) {
                            boolean roomBusy = conflicts.stream().anyMatch(e -> e.getWeek() == currentW
                        ...
                        if (!availableRooms.isEmpty()) {
                            ...
                            String weekLabel = (startWeek != null && currentW == startWeek) ? "This Week"
                                    : (startWeek != null && currentW == startWeek + 1) ? "Next Week"
                                            : "Week " + currentW;

                            slot.description = weekLabel + " - " + dateStr + "(" + daysOfWeek[d - 1] + ") - Period "
                                    + p;
    ...
    public List<RoomOccupationDto> getLiveRoomOccupation(java.time.LocalDateTime clientTime) {
        ...
        // Calculate period based on time (8:00 start, 90m duration, 15m break, lunch
        // 13-14)
        ...
        if (!time.isBefore(p1Start) && time.isBefore(p1End))
            targetPeriod = 1;
        else if (!time.isBefore(p2Start) && time.isBefore(p2End))
            targetPeriod = 2;
        else if (!time.isBefore(p3Start) && time.isBefore(p3End))
            targetPeriod = 3;
        else if (!time.isBefore(p4Start) && time.isBefore(p4End))
            targetPeriod = 4;
        else if (!time.isBefore(p5Start) && time.isBefore(p5End))
            targetPeriod = 5;
        ...
        for (Room room : allRooms) {
            ...
            Event current = null;
            if (targetPeriod > 0 && targetWeek > 0 && targetWeek <= active.getWeeks() && targetDay >= 1
                    && targetDay <= 6) {
            ...
            if (current != null) {
                ...
                if (current.getCourseId() != null)
                    courseRepo.findById(current.getCourseId()).ifPresent(c -> eventDto.courseName = c.getName());
                if (current.getSectionId() != null)
                    batchRepo.findSectionById(current.getSectionId())
                            .ifPresent(s -> eventDto.sectionName = s.getName());
                if (current.getBatchId() != null)
                    batchRepo.findById(current.getBatchId()).ifPresent(b -> eventDto.batchName = b.getName());
    ...
    private Event findEventForMap(List<Event> events, String roomId, int week, int day, int period) {
        // Find the event for this room, day, period. Don't exclude CANCELED because the
        // UI needs to show it as unoccupied but with info.
        return events.stream()
                .filter(e -> roomId.equals(e.getRoomId()) && e.getWeek() == week && e.getDay() == day
                        && e.getPeriod() == period)
                .findFirst()
                .orElse(null);
    }
